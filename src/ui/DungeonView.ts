...
export class DungeonView {
    ...
    private getPositionsAtDepth(depth: number): Array<{x: number, y: number, screenX: number, screenY: number, side: string}> {
        const positions: Array<{x: number, y: number, screenX: number, screenY: number, side: string}> = [];
        ...
        for (let i = -1; i <= 1; i++) {
            ...
            positions.push({ x, y, screenX, screenY, side: i === 0 ? 'front' : (i < 0 ? 'left' : 'right') });
        }
    ...
    private canSeeWall(x: number, y: number, side: string): boolean {
        ...
        switch (side) {
            case 'front':
                switch (this.playerFacing) {
                    case 'north': return tile.northWall;
                    case 'south': return tile.southWall;
                    case 'east': return tile.eastWall;
                    case 'west': return tile.westWall;
                }
                break;
            case 'left':
                switch (this.playerFacing) {
                    case 'north': return tile.westWall;
                    case 'south': return tile.eastWall;
                    case 'east': return tile.northWall;
                    case 'west': return tile.southWall;
                }
                break;
            case 'right':
                switch (this.playerFacing) {
                    case 'north': return tile.eastWall;
                    case 'south': return tile.westWall;
                    case 'east': return tile.southWall;
                    case 'west': return tile.northWall;
                }
                break;
        }
