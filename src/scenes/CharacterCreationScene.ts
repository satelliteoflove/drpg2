...
export class CharacterCreationScene extends Scene {
    ...
    private classes: CharacterClass[] = ['Fighter', 'Mage', 'Priest', 'Thief', 'Bishop', 'Samurai', 'Lord', 'Ninja'];
    ...
    private renderClassStep(ctx: CanvasRenderingContext2D): void {
        ...
        this.classes.forEach((charClass, index) => {
            ...
            ctx.fillStyle = index === this.selectedIndex ? '#ffff00' : 
                           canSelect ? '#fff' : '#666';
    ...
    private renderConfirmStep(ctx: CanvasRenderingContext2D): void {
        ...
        if (this.currentCharacter.stats) {
            ...
            ctx.fillText(`STR: ${this.currentCharacter.stats.strength}  INT: ${this.currentCharacter.stats.intelligence}  PIE: ${this.currentCharacter.stats.piety}`, 100, 300);
            ctx.fillText(`VIT: ${this.currentCharacter.stats.vitality}  AGI: ${this.currentCharacter.stats.agility}  LUK: ${this.currentCharacter.stats.luck}`, 100, 320);
        }
    ...
    private getRaceDescription(race: CharacterRace): string {
        switch (race) {
            case 'Human': return 'Balanced stats, no bonuses or penalties';
            case 'Elf': return '+2 INT, +1 AGI, -2 VIT';
            case 'Dwarf': return '+2 STR, +3 VIT, -2 AGI';
            case 'Gnome': return '+1 INT, +2 PIE, -2 STR';
            case 'Hobbit': return '+3 AGI, +2 LUK, -3 STR';
            default: return '';
        }
    }
    ...
    private createCharacter(): void {
        if (this.currentCharacter.race && this.currentCharacter.class && this.currentCharacter.alignment) {
