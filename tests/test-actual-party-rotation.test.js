...
test('test actual Party class rotation', async ({ page }) => {
  ...
  const rotationTest = await page.evaluate(() => {
    ...
    if (window.game?.gameState?.party) {
      ...
      if (typeof party.move === 'function') {
        party.move('right');
        testResults.push({
          test: 'game party right from north',
          expected: 'east',
          actual: party.facing,
        });
        ...
        party.move('right');
        testResults.push({
          test: 'game party right from east',
          expected: 'south',
          actual: party.facing,
        });
        ...
        party.move('left');
        testResults.push({
          test: 'game party left from south',
          expected: 'east',
          actual: party.facing,
        });
      } else {
        testResults.push({
          test: 'party.move is function',
          expected: true,
          actual: typeof party.move,
        });
      }
    }
  ...
  rotationTest.forEach((result) => {
    ...
    console.log(
      `  ${passed ? '✓' : '✗'} ${result.test}: expected "${result.expected}", got "${result.actual}"`
    );
  });
