...
export class LayerTestUtils {
  ...
  static renderDebugOverlay(renderManager: RenderManager): void {
    ...
    for (const layer of layers) {
      ...
      ctx.fillText(
        `${contentIndicator} ${layer.name} (z:${layer.zIndex}) ${layer.isDirty ? 'DIRTY' : 'CLEAN'}`,
        15,
        y
      );
      ...
      if (layer.contentBounds) {
        ...
        ctx.fillText(
          `  Content: ${layer.contentBounds.minX},${layer.contentBounds.minY} to ${layer.contentBounds.maxX},${layer.contentBounds.maxY}`,
          15,
          y + 12
        );
        y += 24;
