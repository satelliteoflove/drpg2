  ...
  test('debug ASCII state initialization', async ({ page }) => {
    ...
    await page.evaluate(() => {
      if (window.FeatureFlags) {
        ...
        console.log(
          'DUNGEON_ASCII enabled:',
          window.FeatureFlags.isEnabled('DUNGEON_ASCII', 'Dungeon')
        );
      }
    });
      ...
      if (scene) {
        ...
        if (ctx) {
          ...
          if (scene.getASCIIState) {
            console.log(
              'ASCII State after render:',
              scene.getASCIIState() ? 'initialized' : 'null'
            );
          }
        }
      }
    ...
    const gridContent = await page.evaluate(() => {
      ...
      if (scene && scene.getASCIIState) {
        ...
        if (asciiState) {
          ...
          const sample = grid.cells
            .slice(0, 5)
            .map((row) => row.slice(0, 20).join(''))
            .join('\n');
          return {
            width: grid.width,
            height: grid.height,
