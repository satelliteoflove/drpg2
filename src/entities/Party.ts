...
import { IParty, Direction, Formation } from '../types/GameTypes';
...
export class Party implements IParty {
    ...
    public swapCharacters(index1: number, index2: number): void {
        if (index1 < 0 || index1 >= this.characters.length ||
            index2 < 0 || index2 >= this.characters.length) {
            return;
        }
        
        [this.characters[index1], this.characters[index2]] = 
        [this.characters[index2], this.characters[index1]];
    }
