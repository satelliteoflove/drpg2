...
test.describe('DungeonScene ASCII Rendering', () => {
  ...
  test('should render first-person view with proper depth', async ({ page }) => {
      ...
      if (scene && scene.getASCIIState) {
        ...
        if (asciiState) {
          ...
          return {
            ...
            hasWalls:
              gridString.includes('#') || gridString.includes('▓') || gridString.includes('▒'),
    ...
    expect(
      depthRendering.hasWalls ||
        depthRendering.hasFarShading ||
        depthRendering.hasMidShading ||
        depthRendering.hasNearShading
    ).toBeTruthy();
  });
