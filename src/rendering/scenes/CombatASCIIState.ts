...
export class CombatASCIIState extends BaseASCIIScene {
  ...
  private drawMonsters(grid: ASCIIState, monsters: Monster[]): void {
    ...
    monsters.forEach((monster, index) => {
      ...
      // Monster name
      const displayName = monster.name.length > 12 ? 
        monster.name.substring(0, 12) + '.' : monster.name;
      ...
      const barColor = hpPercent > 0.5 ? '#00FF00' : 
                       hpPercent > 0.25 ? '#FFAA00' : '#FF0000';
  ...
  private drawPartyFormation(grid: ASCIIState): void {
    ...
    aliveCharacters.forEach((char: any, index: number) => {
      ...
      const displayName = char.name.length > 8 ? 
        char.name.substring(0, 8) + '.' : char.name;
      grid.writeText(partyX + 4, y, displayName, { foreground: '#FFFFFF' });
      
      // HP indicator
      const hpPercent = char.stats.hp / char.stats.maxHp;
      const hpColor = hpPercent > 0.5 ? '#00FF00' : 
                      hpPercent > 0.25 ? '#FFAA00' : '#FF0000';
  ...
  private getClassSymbol(className: string): string {
    switch (className.toLowerCase()) {
      case 'fighter': return 'F';
      case 'mage': return 'M';
      case 'priest': return 'P';
      case 'thief': return 'T';
      default: return '@';
    }
  }
  ...
  private drawPartyStatus(grid: ASCIIState): void {
    ...
    party.forEach((char: any) => {
      ...
      grid.writeText(xOffset + 3, y + 1, `${char.stats.hp}/${char.stats.maxHp}`, { foreground: hpColor });
      
      grid.writeText(xOffset, y + 2, 'MP:', { foreground: '#AAAAAA' });
      grid.writeText(xOffset + 3, y + 2, `${char.stats.mp}/${char.stats.maxMp}`, { foreground: '#6666FF' });
    ...
    if (this.actionState === 'select_action') {
      ...
      this.actionOptions.forEach((option, index) => {
        ...
        const textColor = option.enabled ? 
          (isSelected ? '#FFAA00' : '#FFFFFF') : '#666666';
      ...
      grid.writeText(4, menuY + 3, 'Use LEFT/RIGHT arrows to select target', { foreground: '#AAAAAA' });
  ...
  private selectPreviousAction(): void {
    this.selectedAction = (this.selectedAction - 1 + this.actionOptions.length) % this.actionOptions.length;
    this.initializeCombatLayout();
  }
  ...
  public setActionState(state: 'select_action' | 'select_target' | 'select_spell' | 'waiting'): void {
