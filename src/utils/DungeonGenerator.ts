import { DungeonLevel, DungeonTile, EncounterZone, DungeonEvent } from '../types/GameTypes';
...
export class DungeonGenerator {
    ...
    private createCorridor(tiles: DungeonTile[][], x1: number, y1: number, x2: number, y2: number): void {
        let x = x1;
        let y = y1;
    ...
    private getRandomEventType(): 'message' | 'trap' | 'treasure' | 'teleport' | 'spinner' | 'darkness' {
        const types: ('message' | 'trap' | 'treasure' | 'teleport' | 'spinner' | 'darkness')[] = 
            ['message', 'trap', 'treasure', 'teleport', 'spinner', 'darkness'];
