...
export class PerformanceMonitor {
    ...
    public markRenderEnd(): void {
        ...
        if (metrics && metrics.frameMetrics.length > 0) {
            ...
            metrics.frameMetrics[metrics.frameMetrics.length - 1].renderTime = Math.round(renderTime * 100) / 100;
        }
    }
    ...
    public markUpdateEnd(): void {
        ...
        if (metrics && metrics.frameMetrics.length > 0) {
            ...
            metrics.frameMetrics[metrics.frameMetrics.length - 1].updateTime = Math.round(updateTime * 100) / 100;
        }
    }
    ...
    public generateReport(): PerformanceReport {
        ...
        const overallAverageFPS = allFPSValues.length > 0 
            ? allFPSValues.reduce((a, b) => a + b, 0) / allFPSValues.length 
            : 0;
        
        const overallMinFPS = allFPSValues.length > 0 
            ? Math.min(...allFPSValues) 
            : 0;
        
        const overallMaxFPS = allFPSValues.length > 0 
            ? Math.max(...allFPSValues) 
            : 0;
        ...
        DebugLogger.info('PerformanceMonitor', `Generated performance report. Score: ${performanceScore.toFixed(1)}/100`);
    ...
    public exportReportAsMarkdown(report: PerformanceReport): string {
        ...
        lines.push(`- **Dropped Frames**: ${report.globalMetrics.droppedFrames} (${((report.globalMetrics.droppedFrames / report.globalMetrics.totalFramesRendered) * 100).toFixed(1)}%)`);
        
        // Add frame drop analysis
        if (report.globalMetrics.droppedFrames > 0) {
            const dropRate = (report.globalMetrics.droppedFrames / report.globalMetrics.totalFramesRendered) * 100;
            if (dropRate > 50) {
        ...
        report.sceneMetrics.forEach((metrics) => {
            ...
            if (metrics.peakMemoryUsage !== undefined) {
                lines.push(`- **Peak Memory Usage**: ${(metrics.peakMemoryUsage / 1024 / 1024).toFixed(1)}MB`);
            }
            ...
            if (metrics.frameMetrics.length > 0) {
                ...
                lines.push(`  - 60+ FPS: ${fps60plus} frames (${(fps60plus / metrics.frameMetrics.length * 100).toFixed(1)}%)`);
                lines.push(`  - 30-60 FPS: ${fps30to60} frames (${(fps30to60 / metrics.frameMetrics.length * 100).toFixed(1)}%)`);
                lines.push(`  - <30 FPS: ${fpsBelow30} frames (${(fpsBelow30 / metrics.frameMetrics.length * 100).toFixed(1)}%)`);
                ...
                if (maxConsecutiveDrops > 5) {
                    lines.push(`  - **⚠️ Stuttering detected**: Max ${maxConsecutiveDrops} consecutive dropped frames`);
                }
            }
    ...
    private finalizeSceneMetrics(sceneName: string): void {
        ...
        DebugLogger.info('PerformanceMonitor', 
