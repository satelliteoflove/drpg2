...
export class InventoryScene extends Scene {
  ...
  private mode: 'character_select' | 'inventory' | 'equipment' | 'trade_select' | 'trade_target' =
    'character_select';
  ...
  private renderCharacterSelect(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.font = '12px monospace';
    ctx.fillText(
      'I: View Inventory | E: View Equipment | T: Trade Items',
      10,
  ...
  private renderTradeTarget(ctx: CanvasRenderingContext2D): void {
    ...
    this.gameState.party.characters.forEach((character: Character, index: number) => {
      ...
      ctx.fillText(
        `${index + 1}. ${character.name}${status} (${weight}/${capacity} weight)`,
        20,
        y
      );
    });
    ...
    if (key === KEY_BINDINGS.inventory.moveUp || key === KEY_BINDINGS.menu.up || key === KEY_BINDINGS.menu.alternateUp) {
      ...
      if (item && item.equipped) {
        ...
        if (equipSlot && InventorySystem.unequipItem(character, equipSlot)) {
          ...
          this.gameState.messageLog?.addWarningMessage(
            `Cannot remove ${item.name} - it is cursed!`
          );
        }
      }
      return true;
  ...
  private handleTradeTarget(key: string): boolean {
    ...
    if (key === KEY_BINDINGS.inventory.moveUp || key === KEY_BINDINGS.menu.up || key === KEY_BINDINGS.menu.alternateUp) {
      ...
      do {
        ...
        this.selectedCharacter === this.tradeFromCharacter &&
        this.selectedCharacter < characters.length - 1
      );
      return true;
    } else if (key === 'enter') {
      ...
      if (InventorySystem.tradeItem(fromCharacter, toCharacter, this.tradeItem)) {
        ...
        this.gameState.messageLog?.addWarningMessage(
          'Trade failed - target cannot carry more weight'
        );
      }
